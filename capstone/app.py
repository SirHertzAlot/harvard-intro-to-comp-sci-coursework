...
def register():
    ...
    if request.method == "POST":
        ...
        if password == confirmation and len(password) > 0 and len(username) > 0:
            ...
            db.execute("INSERT INTO users (user_email, user_username, user_password, user_bio, user_city, user_state, user_country) VALUES(?, ?, ?, ?, ?, ?, ?)", email, username, hash, bio, city, state, country)
